...
# wrapping save_map
g1 <- save_seq(frame1, z.name="z", id=id, ortho=FALSE, n.frames=n, col=pal, type="maptiles", z.range=rng, save.plot=FALSE, return.plot=TRUE)
g2 <- save_seq(frame1, z.name="z", id=id, n.frames=n, col=pal, type="maptiles", z.range=rng, save.plot=FALSE, return.plot=TRUE)

data(borders)
borders <- map(1:n, ~mutate(borders, idx = .x)) %>% bind_rows

gg <- save_seq(borders, id=id, n.frames=n, col="purple", type="maplines", save.plot=FALSE, return.plot=TRUE)

# wrapping save_ts
x <- group_by(temps, Year, idx) %>% summarise(z=mean(z))
xlm <- range(x$Year)
ylm <- range(x$z)
gg2 <- save_seq(filter(x, idx <= 3), style="tsline", x="Year", y="z", id=id, col="blue", xlm=xlm, ylm=ylm, save.plot=FALSE, return.plot=TRUE)
