...
Last of all, but important, the data frame must contain a column of still frame sequence IDs.
This is simply the integers `1, 2, ..., n` for `n` frames or individual plots.
...
In the current version of `mapmate` this column in renamed to `frameID` internally during processing for some conveniences with internal coding.
While this may not create much appreciable overhead, it can still be avoid if the column name is already `frameID`.
