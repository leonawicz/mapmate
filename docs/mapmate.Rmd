...
<!-- mapmate.md is generated from mapmate.Rmd. Please edit that file -->

```{r, echo = FALSE}
knitr::opts_chunk$set(cache=TRUE, collapse=TRUE, comment=">", fig.path="README-Intro-", message=F, warning=F, error=F, eval=F, tidy=T)
```
...
For simplicity, set the number of still frames to be equal to the number of unique time points in the data frame, in example this one frame for each year.
...
and since we are not saving files to disk we can also ignore arguments like `dir` and `file`, but we will return to all these immediately after.

### Initial static 2D and 3D map examples

```{r, eval=TRUE, fig.show='hold', fig.width=0.8*5.33, fig.height=0.8*3, fig.cap="2D flat map and 3D globe"}
...
The optional `file` argument is also used.
...
walk(temps, ~save_map(.x, z.name="z", id=id, ortho=FALSE, col=pal, type="maptiles", file=file, z.range=rng))
...
```{r}
file <- "annual_3D_fixed"
walk(temps, ~save_map(.x, z.name="z", id=id, lon=rep(-70, n), lat=50, n.period=n, n.frames=n, col=pal, type="maptiles", file=file, z.range=rng))
...
file <- "borders_3D_rotating"
walk(borders, ~save_map(.x, id=id, lon=-70, lat=50, n.period=30, n.frames=n, col="orange", type="maplines", file=file))
...
file <- "year1_3D_rotating"
walk(temps1, ~save_map(.x, z.name="z", id=id, lon=-70, lat=50, n.period=30, n.frames=n, col=pal, type="maptiles", file=file, z.range=rng1))
...
```{r}
file <- "annual_3D_rotating"
walk(temps, ~save_map(.x, z.name="z", id=id, lon=-70, lat=50, n.period=30, n.frames=n, col=pal, type="maptiles", file=file, z.range=rng))
...
walk(bath, ~save_map(.x, z.name="z", id=id, n.frames=n, col=pal_bath, type="maptiles", file="background", z.range=rng_bath))
walk(borders, ~save_map(.x, id=id, n.frames=n, col="black", type="maplines", file="foreground"))
walk(temps, ~save_map(.x, z.name="z", id=id, n.frames=n, col=pal, type="maptiles", file="timeseries", z.range=rng))
...
```{r}
library(parallel)
mclapply(bath, save_map, z.name="z", id=id, n.frames=n, col=pal_bath, type="maptiles", file="background", z.range=rng_bath, mc.cores=32)
mclapply(borders, save_map, id=id, n.frames=n, col="orange", type="maplines", file="foreground", mc.cores=32)
mclapply(temps, save_map, z.name="z", id=id, n.frames=n, col=pal, type="maptiles", file="timeseries", z.range=rng, mc.cores=32)
...
```{r}
# Serial
save_seq(bath, z.name="z", id=id, n.frames=n, col=pal_bath, type="maptiles", file="background", z.range=rng_bath)
save_seq(borders, id=id, n.frames=n, col="black", type="maplines", file="foreground")
save_seq(temps, z.name="z", id=id, n.frames=n, col=pal, type="maptiles", file="timeseries", z.range=rng)

# Parallel, Linux, with 32 CPU cores available
save_seq(bath, use_mclapply=TRUE, mc.cores=32, 
  z.name="z", id=id, n.frames=n, col=pal_bath, type="maptiles", file="background", z.range=rng_bath)
save_seq(borders, use_mclapply=TRUE, mc.cores=32, 
  id=id, n.frames=n, col="black", type="maplines", file="foreground")
save_seq(temps, use_mclapply=TRUE, mc.cores=32, 
  z.name="z", id=id, n.frames=n, col=pal, type="maptiles", file="timeseries", z.range=rng)
...
```{r}
# Walk over all maps
walk(bio, ~save_map(.x, z.name=x1, id=id, lon=0, lat=20, n.period=n, n.frames=n, col=clrs, type="polygons", file=file))
...
file <- "bioDivMaptiles_3D_rotating"

# Return a test map
save_map(bio2[[1]], z.name=x1, id=id, lon=0, lat=20, n.period=n, n.frames=n, col=clrs, type="maptiles", file=file, save.plot=FALSE, return.plot=TRUE)
```

```{r}
# Walk over all maps
walk(bio2, ~save_map(.x, z.name=x1, id=id, lon=0, lat=20, n.period=n, n.frames=n, col=clrs, type="maptiles", file=file))
