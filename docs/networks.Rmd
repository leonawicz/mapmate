...
The first example ues a flat map. Therefore it is important to break lines at the international dateline when preparing the data.
...
Finally, `gc_paths` is used to further exand the great circle arcs data frame into one that contains sequences of ordered segments along each arc.
The segments can vary in length between distinct arcs and can overlap one another within an arc.
The `group` argument is required to identify distinct arcs in the input data frame. `size` is required to set an upper limit on the number of points contituting an arc segment;
...
Note that I still included the `png.args` argument even though `width` and `height` will be discared
...
gglist <- save_seq(paths_flat, id="id", n.frames=n, ortho=FALSE, type="network", ylim=ylm, suffix="2D", png.args=png.args, save.plot=FALSE, return.plot=TRUE)
...
save_seq(paths_flat, id="id", n.frames=n, ortho=FALSE, type="network", suffix="2D", png.args=png.args)
...
gglist <- save_seq(paths_globe, id="id", n.frames=n, col=clrs, type="network", pt.size=c(1,1,3,2), suffix="3D", png.args=png.args, save.plot=FALSE, return.plot=TRUE)
