...
```{r, echo = FALSE}
knitr::opts_chunk$set(cache=TRUE, collapse=TRUE, comment=">", fig.path="README-ffmpeg-", message=F, warning=F, error=F, eval=T, tidy=T)
dir.create("test-images", showWarnings=FALSE)
files <- list.files("test-images", pattern="*.gif|*.mp4", full.names=TRUE)
unlink(files)
```
...
save_seq(paths, id="id", dir=dir, n.frames=n, col=clrs, type="network", pt.size=size, file=file[1], png.args=args1)
save_seq(paths, id="id", dir=dir, n.frames=n, col=clrs, type="network", pt.size=size, file=file[2], png.args=args2)
```
...
save_seq(borders, id="id", dir=dir, n.frames=n, col="dodgerblue", type="maplines", file=file[1], png.args=args1)
save_seq(borders, id="id", dir=dir, n.frames=n, col="dodgerblue", type="maplines", file=file[2], png.args=args2)
```
...
out <- "network_10fps.gif"
# 10 fps gif
ffmpeg(pattern=p, output=out, output_dir=dir, rate=10)
# Same as above. Set overwrite=TRUE if output exists.
ffmpeg(pattern=p, output=out, output_dir=dir, delay=1/10, overwrite=TRUE)
```
...
out <- "network_10fps_stacking.gif"
# The stacking of images is revealed by the transparent backgrounds
ffmpeg(pattern=p, output=out, output_dir=dir, rate=10)
```

Now that that has been shown, let's make mp4 files. The next example below uses the default `rate="ntsc"`, or about 29.97 fps.
Images in the sequence can be skipped using `start`.

```{r ffmpeg3}
p <- "maplines_bg_black_%04d.png"
out <- "borders_start60.mp4"
# start on frame 60
ffmpeg(pattern=p, output=out, output_dir=dir, start=60)
```
...
```{r ffmpeg4}
p <- "maplines_bg_black_%04d.png"
out <- c("borders_300x300.mp4", "borders_720x480.mp4")
ffmpeg(pattern=p, output=out[1], output_dir=dir, size="300x300")
ffmpeg(pattern=p, output=out[2], output_dir=dir, size="ntsc")
```
...
p <- "network_%04d.png"
out <- "network.mp4"
ffmpeg(pattern=p, output=out, output_dir=dir, rate="ntsc", size="ntsc", preset="ultrafast", lossless=TRUE, overwrite=TRUE)
ffmpeg(pattern=p, output=out, output_dir=dir, rate=5, size="source", preset="veryslow", lossless=TRUE, min.rate="ntsc", overwrite=TRUE)
ffmpeg(pattern=p, output=out, output_dir=dir, size="300x300", preset="veryslow", overwrite=TRUE)
ffmpeg(pattern=p, output=out, output_dir=dir, delay=1, fps.out=10, overwrite=TRUE)
```
...
ffmpeg(pattern=p, output="layered.mp4", output_dir=dir, alpha=1)
ffmpeg(pattern=rev(p), output="layered_reversed.mp4", output_dir=dir)
ffmpeg(pattern=rev(p), output="layered_reversed_alpha50.mp4", output_dir=dir, alpha=0.5)
```
