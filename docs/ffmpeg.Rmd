...
The `borders` data set is also used for a couple versions of a map backgroud layer.
...
Some example `ffmpeg` calls specify gif output so that those gifs may be shown here in line with the tutorial code and documentation,
but most of the calls focus on standard video output file types such as mp4.
...
Be sure to check the [introduction vignette](mapmate.html#Parallel-Processing) first if you are not familair with the restrictions.
...
We continue to use gifs here because the output is embedded in this document immediately following the call for you to see the result,
even though some settings used are more commonly associated with video formats and not with animated gifs.
Several `ffmpeg` arguments are not relevant to gifs and will be seen in later examples when we switch from gifs to mp4 file outputs.
...
ffmpeg(pattern=p, output="layered.gif")
ffmpeg(pattern=rev(p), output="layered_reversed.gif")
ffmpeg(pattern=rev(p), output="layered_reversed_alpha50.gif", alpha=0.5)
```
