...
```{r, echo = FALSE}
knitr::opts_chunk$set(collapse=TRUE, comment="#>", fig.path = "README-", message=F, warning=F, error=F, tidy=T)
```
...
The motivation for the package is to assist with heavy data processing and image production for large, complex data animation videos that can require a signifcant amount of computing resources to create.
...
They also show how some of the options for map types included `save_map` are primarily included to reveal their current limitation in drawing maps.
Points, paths and tiles tend to work well, as do contour lines, but polygons often display serious clipping issues.

Also, since these examples are simply intended to display what kind of map outputs are made given the various arguments that can be passed to `save_map`,
the plot or image frame number or ID value never goes beyond `1` and `save_map` is not called iteratively.
For code related to typical map sequence generation see the introductory vignette for examples: `browseVignettes(package="mapmate")` or [view it here](https://leonawicz.github.io/mapmate/mapmate.html).
...
hence why every call to `save_map` in this set of examples must explcitly include the non-default `save.plot` and `return.plot` arguments.
...
Explicit contour lines can also be overlayed with the base map type.
...
can also be plotted by excluding `z.name`.
...
bio2 <- rasterize(spdf, r, field=x1) %>% rasterToPoints %>%
  tbl_df() %>% setNames(c("lon", "lat", x1)) %>% mutate(frameID = 1))

save_map(bio, z.name="z", id=id, lon=-70, lat=50, type="polygons", save.plot=FALSE, return.plot=TRUE)

save_map(bio2, z.name="z", id=id, lon=-70, lat=50, type="maptiles", save.plot=FALSE, return.plot=TRUE)
```
